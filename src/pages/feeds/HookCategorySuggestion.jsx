...
import { HookCategorySuggestionSkeleton } from "./HookCategorySuggestionSkeleton";

export const HookCategorySuggestion = () => {
  const [isFetchingTags, setIsFetchingTags] = useState(true);
  const [hookCategory, setHookCategory] = useState(null);

  const hookCategorySample = [
    {
      id: "101",
  ...
  useEffect(() => {
    setTimeout(() => {
      setHookCategory(hookCategorySample);
  ...
  return (
    <React.Fragment>
      ...
      {isFetchingTags && <HookCategorySuggestionSkeleton count={4} />}
